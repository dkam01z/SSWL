...
app.config['MYSQL_PASSWORD'] = 'SPDASDDLP334DZX'
...
def dashboard():
    if request.method == 'GET':
        ...
        result = cur.execute("Select tb2.*, tb3.PARENT_ID , tb4.email FROM EVENTS tb2 LEFT JOIN student_event tb1 ON tb2.EVENT_ID = tb1.EVENT_ID RIGHT JOIN STUDENT tb3 ON tb1.student_id = tb3.STUDENT_ID RIGHT JOIN PARENTS tb4 ON tb3.PARENT_ID = tb4.Parent_id WHERE applied=0 and email=%s", [session['email']] )
        events =cur.fetchall()
...
def add_event():
    ...
    if request.method == 'POST' and form.validate():
        ...
        cur.execute("INSERT INTO events(eventname, body,fee,timefrom, timeto,event_date,creator,Number_of_days,ygroup,eventtype_id) VALUES (%s, %s,%s,%s,%s,%s,%s,%s,%s, LAST_INSERT_ID())", ( eventname, body,fee,time_from, time_to,event_date,session['email'],Number_of_days,ygroup))
        cur.execute("SELECT * FROM EVENTS WHERE eventname = %s", [eventname])
 
        cur.execute("select student_id from student" )
        all = cur.fetchall()
        for one in all:
            cur.execute("insert into student_event(student_id, event_id, applied,eventname) VALUES (%s,LAST_INSERT_ID(),%s,%s) " , (one, False,eventname))
...
def apply(EVENT_ID):
    if request.method == 'POST':
        ...
        cur.execute(" INSERT INTO parvent(PARENT_ID,EVENT_ID,trip_date ,eventname) VALUES (%s,%s,%s,%s) ", (parent_id,EVENT_ID,event_date  , eventname) )
        cur.execute("UPDATE STUDENT_EVENT tb2 LEFT JOIN STUDENT tb1 ON tb1.STUDENT_ID = tb2.student_id RIGHT JOIN events TB3 on tb3.event_id = TB2.event_id RIGHT JOIN parents tb4 on tb4.parent_id = tb1.parent_id SET APPLIED = True WHERE tb2.eventname=%s and email = %s", (eventname, session['email'] ,))
        # Commit
        mysql.connection.commit()
...
if __name__ == '__main__':
    app.secret_key = 'SPDASDDLP334DZX'
